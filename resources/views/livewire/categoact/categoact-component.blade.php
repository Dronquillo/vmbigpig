<div>
    <x-card cardTitle='Gestion Categoria Activo ({{$this->totalRegistros}})' >
        <x-slot:cardTools>  
            <a href="#" class="btn btn-primary" wire:click='create'>
                <i class="fas fa-plus-circle"></i>  Crear Categoria Activo</a>
        </x-slot>
        ...
        <x-table>
            ...
            @forelse($categoacts as $category)
                <tr>
                    <td>{{$category->id}}</td>
                    <td>{{$category->nombre}}</td>
                    <td>
                        <a href="{{route('categoact.show',$category)}}" class="btn btn-sm btn-success" title="Ver"><i class="far fa-eye"></i></a>
    ...
    <x-modal modalId="modalCategoact" modalTitle="Categorias">
